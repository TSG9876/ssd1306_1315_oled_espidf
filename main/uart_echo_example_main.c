...
static void SSD1315_INIT()
{
    ...
    for (int i = 0; i < (14); i++)
    {
        ...
        if (array_size == 1)
        {
            ...
            ret = i2c_master_transmit(dev_handle, (uint8_t *)data1, 2, -1);
            vTaskDelay(50 / portTICK_PERIOD_MS);

            if (ret != ESP_OK)
            {
                printf("i2c command failed: %d", ret);
            }
...

        {
            ...
            ret = i2c_master_transmit(dev_handle, (uint8_t *)data1, 3, -1);
            vTaskDelay(50 / portTICK_PERIOD_MS);
...

        {
            ...
            ret = i2c_master_transmit(dev_handle, (uint8_t *)data1, 4, -1);
            vTaskDelay(50 / portTICK_PERIOD_MS);
...
static void clear_oled()
{
    ...
    // ESP_ERROR_CHECK(i2c_master_transmit(dev_handle, col_cmd, sizeof(col_cmd), pdMS_TO_TICKS(10)));
    ...
    uint8_t *data1 = malloc(3);

    uint8_t *data2 = malloc(2);

    data1[0] = UINT8_C(0x00);
    data1[1] = UINT8_C(0x8D);
    data1[2] = UINT8_C(0x14);

    ret = i2c_master_transmit(dev_handle, data1, 3, -1);
    vTaskDelay(50 / portTICK_PERIOD_MS);

    data2[0] = UINT8_C(0x00);
    data2[1] = UINT8_C(0xAF);
    ret = i2c_master_transmit(dev_handle, data2, 2, -1);
    vTaskDelay(50 / portTICK_PERIOD_MS);
}
        ...
        /*   printf("ST TRF\n");
          ...
          for (int i = 0; i < arg_int; i++)
          {
            //  fdata1[idx] = *(uint8_t *)(arg_ptr) + i;
              printf("arg_ptr[ %i ]   %02x \t", i, *(uint8_t *)(arg_ptr) + i);
              idx++;
          } */
               ...
               for (int i = 0; i < buf_idx; i++)
                  {
                      ...
                      printf("buffer[%i]--> %02x \t", i,buffer[i]);
                    }
...
/* void u8x8_display_init(u8x8_t *u8x8) {
  uint8_t i = 0;
  uint8_t *data = (uint8_t *)u8x8_ssd1315_init_seq;

  while(data[i] != U8X8_END()) {
    if(data[i] == U8X8_MSG_CAD_SEND_CMD) {
      u8x8_SendCommand(u8x8, data[i+1]);
      i++;
    }
    else if(data[i] == U8X8_MSG_CAD_SEND_DATA) {
      u8x8_SendData(u8x8, 1, &data[i+1]);
      i++;
    }
    else if(data[i] == U8X8_MSG_DELAY_MILLI) {
      vTaskDelay(pdMS_TO_TICKS(data[i+1]));
      i++;
    }
    i++;
  }
...
void ssd1315_custom_init(u8x8_t *u8x8)
{
    u8x8_ClearDisplay(u8x8);

    // Send critical SSD1315 commands
    u8x8_cad_StartTransfer(u8x8);

    // Column offset fix
    u8x8_cad_SendCmd(u8x8, 0x21); // Set column address
    u8x8_cad_SendArg(u8x8, 0x00); // Start column = 3
    u8x8_cad_SendArg(u8x8, 0x7F); // End column = 130

    // Page addressing setup
    u8x8_cad_SendCmd(u8x8, 0x22); // Set page address
    u8x8_cad_SendArg(u8x8, 0x00); // Start page = 0
    u8x8_cad_SendArg(u8x8, 0x07); // End page = 7

    u8x8_cad_EndTransfer(u8x8);
    vTaskDelay(pdMS_TO_TICKS(50));
}

void draw_frame_box(uint8_t a)
{
...
void u8g2_task()
{
    ...
    /*   u8g2_SendF(&u8g2, "c", 0x000);
      u8g2_SendF(&u8g2, "c", 0x010);
      u8g2_SendF(&u8g2, "c", 0x0B0);
      u8g2_SendF(&u8g2, "ca", 0x020, 0x010);
      u8g2_SendF(&u8g2, "caa", 0x021, 0x000, 0x07F);

      u8g2_SendF(&u8g2, "caa", 0x022, 0x000, 0x007); */

    u8g2_InitDisplay(&u8g2);
    u8g2_SetPowerSave(&u8g2, 0);


        ESP_LOGI("MEMORY", "Free memory AFTER  INIT: %lu bytes\n", esp_get_free_heap_size());
  ...
  ESP_LOGI("MEMORY", "Free memory AFTER DARW: %lu bytes \n", esp_get_free_heap_size());
    //  SSD1315_INIT();
    //   clear_oled();
    ...
    // u8x8_Setup(&u8g2, u8x8_d_ssd1315_128x64_noname, u8x8_cad_ssd13xx_fast_i2c, u8x8_byte_hw_i2c,  u8x8_dummy_cb);
    ...
    //  vTaskDelay(1000 / portTICK_PERIOD_MS);
    // u8x8_SetI2CAddress(&u8g2,0x3C*2);

    // u8x8_InitDisplay(&u8g2);
    // u8x8_SetPowerSave(&u8g2, 0);
    //  ssd1315_custom_init(&u8g2);

    // u8x8_ClearDisplay(&u8g2.);
    char *ostr = "123456789";

    char *ostr1 = "T E S T 123  ";

    char *ostr2 = "T E S T 2";

    // u8x8_SetFont(&u8g2,  u8x8_font_5x8_f );
    //   u8x8_SetFlipMode(&u8g2,0);
    //  u8x8_DrawGlyph(&u8g2, 5, 10, 'Z');
    //    u8x8_DrawString(&u8g2, 10, 8, ostr1);

    //   u8x8_ClearLine(&u8g2,20);

    // u8x8_RefreshDisplay(&u8g2);

    u8g2_SetFont(&u8g2, u8g2_font_6x10_tf); // Large digital font
                                            // u8g2_DrawStr(&u8g2, 1, 10, ostr1);
                                            // u8g2_DrawHLine(&u8g2, 0, 20, 127);

    //  u8g2_DrawCircle(&u8g2, 60, 50, 5, U8G2_DRAW_ALL);
    ...
    /*
        ...
        do
        {
            ...
            do
            {
                ...
                u8g2_SetDrawColor(&u8g2, 1);
                //  u8g2_DrawFrame(&u8g2, 4, 4, 127, 63);
                // u8g2_SetDrawColor(&u8g2, colour);
                ...
                if (u8g2.font == NULL)
                {
                    ESP_LOGE("FONT", "No font loaded!");
                    return;
                }
                else
                {

                    ESP_LOGI("FONT", "Font: %i", u8g2.font_info.glyph_cnt);
                }

                //  u8g2_DrawStr(&u8g2, 10, 15, "6x10");
                // u8g2_SetFont(&u8g2, u8g2_font_10x20_tf);
                //  u8g2_UpdateDisplay(&u8g2);
                //  u8g2_DrawStr(&u8g2, 10, 40, "10x20");

                // u8g2_DrawLine(&u8g2, 5, 5, 127, 63);
            } while (u8g2_NextPage(&u8g2));
